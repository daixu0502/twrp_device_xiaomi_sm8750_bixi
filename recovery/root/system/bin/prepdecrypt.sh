...
wait_for_property() {
	...
	log_print 0 "$1 did not reach value $2."
	return 1
}
...
start_crypto_services() {
	log_print 1 "Waiting for Qualcomm secure listeners..."
	wait_for_property vendor.sys.listeners.registered true || return 1
	log_print 1 "Starting KeyMint..."
	setprop ctl.start vendor.keymint-qti
	wait_for_service vendor.keymint-qti running || return 1
	# init.svc=running is published just after exec; give the HAL time to
	# register its KeyMint, SecureClock and SharedSecret binder services.
	sleep 1
	return 0
}
...
finish() {
	...
	if ! start_crypto_services; then
		setprop crypto.ready 0
		log_print 0 "KeyMint failed to become ready."
		exit 3
	fi
	...
	if [ -z "$1" ]; then
		...
		if [ -n "$2" ]; then
			...
			ro.build.version.release)
				default_value=$osver_twrp
				;;
			ro.build.version.security_patch)
				default_value=$patchlevel_twrp
				;;
			*)
				default_value=$1
				;;
			esac
			$setprop_bin "$4" "$default_value"
			log_print 2 "Updating $DEFAULTPROP with default $3..."
			echo "$4=$default_value" >>"/$DEFAULTPROP"
			$5 "$4"
		fi
	else
...
check_encrypt() {
	...
	elif grep -q "fileencryption=" /etc/recovery.fstab 2>/dev/null; then
		# Recovery init may not publish ro.crypto.type before TWRP starts. The
		# bixi fstab is authoritative and explicitly declares FBE v2.
		encrypt_type=file
		setprop ro.crypto.type file
		log_print 1 "File Based Encryption (FBE) detected from recovery.fstab."
	else
